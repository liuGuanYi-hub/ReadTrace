...
> 📌 **阅读须知（2026-09-18 补注）**：本文档是**按时间顺序累积的历史规划档案**，记录了从立项到 P40 各阶段
> 的原始设计与卖点策划。项目在长期迭代中**下线过若干功能**，因此文中部分章节描述的能力**已不存在于当前代码**
> （已知的有：3D 私人展厅、2.5D 空间视差展厅、3D 翻书阅读器、阅读计时器、智能剪贴板嗅探、离线 ISBN 扫码、
> Web 微卡 / DeepLink 微卡、以及早期规划的 FastAPI 自建后端）。
>
> **判断现状请以 `README.md` 与代码为准**，本文档的章节内容仅代表当时的规划意图。
> 已在原文内就地标注的下线项标注为〔⚠️ 已下线〕或【已归档】。
...
> ⚠️ **【已归档 · 2026-09-18 标注】** 下述 FastAPI 后端为早期规划产物，实现已归档至
> `archive/fastapi-backend`，不随 App 分发。**当前架构为 Local-First**：SQLite 单例为唯一真相源，
> 云端同步由 WebDAV 承担，无自建服务端。
...
> ⚠️ **【已归档 · 2026-09-18 标注】** 本章描述的 FastAPI 后端为早期规划产物，实现代码已归档至
> `archive/fastapi-backend`，不再随 App 构建分发。**当前架构为 Local-First**：Android 端以 SQLite
> 单例为唯一真相源，云端同步由 WebDAV（坚果云 / Nextcloud / NAS）双向增量同步承担，无自建服务端。
> 本章内容**保留作为历史规划记录，不代表现状**（T4.5-D 文档层清理）。
...
> ⚠️ **【已归档 · 2026-09-18 标注】** 本节描述的后端目录结构已归档至 `archive/fastapi-backend`，
> 不随 App 构建分发。**当前项目为纯客户端 Local-First 架构**，无自建服务端。
...
- **3D 展厅到阅读器的「破壁穿梭转场」**〔⚠️ 已下线：3D 私人展厅与 3D 翻书阅读器均已移除，见文首说明〕：
...
> ⚠️ **【部分功能已下线 · 2026-09-18 标注】** 本章规划的两项能力**已从代码中移除**，请勿据此判断现状：
> - **智能剪贴板嗅探**（`ClipboardSnifferHelper`）：因剪贴板读取涉及系统权限与隐私合规，已整体删除；速记弹窗现改由调用方经入参 `prefillTitle` 传入标题。
> - **离线 ISBN 扫码**（`IsbnScannerActivity` + Google ML Kit Barcode Scanning）：已删除，同时移除 ML Kit 与 CameraX 依赖（曾致 APK 达 44.5MB，移除后降至 20.9MB）。
>
> 章内其余部分（一句话速记分词、极速入库流）**仍然生效**。本章保留作为历史规划记录。
...
## 46. P36 零成本云端部署与托管选型 (Zero-Cost Deployment & Hosting Stack)

> 立项时间：2026-09-05。固定成本仅域名 29 元/年，其余全部 0 元。

### 46.1 选型背景

项目内多项「待服务端」候选长期挂起：TMDB 电影源（§8 结论为「自建服务端代理后重启评估」）、`cover_server/` 封面服务公网化（当前仅内网可用，封面键 `covers/…` 出门失效）、云端展览社区（v3.0 目前为本地种子数据）、小程序双轨同步的服务端底座（B8 长期方案）。本章节引入一套零成本托管组合，把上述候选全部解锁，同时为 Android 端补上错误监控与统计能力。

### 46.2 服务清单与 ReadTrace 落点

| 序号 | 类别 | 选型 | 成本 | 免费额度 | ReadTrace 落点场景 |
| :-: | :- | :- | :-: | :- | :- |
| 5 | 部署托管 | **EdgeOne Makers**（腾讯全栈应用部署平台） | 0 元 | 40 个项目、云函数 100 万次/月、KV 与 Blob 存储各 1GB；国内访问速度优于 Vercel / Cloudflare Pages | `cover_server/` 公网化试点；TMDB 代理函数；社区广场后端 API |
| 6 | 域名 | 国内云服务商注册 | 29 元/年起 | — | 国内加速与 EdgeOne 国内节点的前提；**需 ICP 备案**（整套方案唯一必须花钱且必须走流程项，一年几十块） |
| 7 | 数据库 | **Supabase** | 0 元 | 500MB 数据库 + 自带用户认证 + 文件存储 | 云端展览社区真实数据底座（替代本地种子）；账号体系云端扩展；小程序双轨同步服务端候选（与微信云开发托管对比评估） |
| 8 | 对象存储 | **七牛云 / 又拍云** | 0 元 | 七牛：实名用户 10GB 存储 + 10GB CDN 回源/月；又拍云：联盟计划网站底部挂 Logo 领 15GB CDN/月 | 预置封面图床（`covers/` 资产公网分发）；用户生成海报/长图的云端托管 |
| 9 | 邮件 | **Resend** | 0 元 | 每月 3000 封、日限 100 封，API 简洁 | 预留：账号找回、重要事件通知（当前账号体系为本地/短信/微信，接入非急迫） |
| 10 | SSL 证书 | **Let's Encrypt** + acme.sh | 0 元 | 自动申请与续期 | 上述全部服务 HTTPS 一次性配置 |
| 11 | 流量统计 | **百度统计 / 51.la** | 0 元 | 百度统计免费适合 SEO；51.la 免费版每月 300 万 PV | Web 微卡 / DeepLink 落地页、小程序 Web 端访问分析 |
| 12 | 错误监控 | **Sentry** | 0 元 | 每月 5000 错误事件，几行代码接入 | Android 端崩溃/异常聚合（Sentry 有官方 Android SDK），替代手工 logcat 排查；Web 端同步覆盖 |

### 46.3 与既有计划的衔接

- **§8 TMDB**：`EdgeOne 云函数 + 域名 + SSL`（序号 5+6+10）即满足「自建服务端代理」的评估前提，TMDB 评估可重启，客户端只换 base URL 与鉴权头；
- **cover_server 公网化**：部署到 EdgeOne 后需设计「内网封面键 `covers/…` → 公网 URL」映射兼容策略，老数据键值不变、解析时按开关拼接公网前缀；
- **B8 小程序同步**：Supabase（序号 7）与微信云开发托管（原 B8 长期方案）二选一对比评估；
- **云端展览社区**：Supabase 认证 + 数据库直接替换 `CommunityRepository` 的本地种子底座，广场互访从「演示数据」升级为真实跨用户数据。

### 46.4 实施路线图 (Phases)

| 阶段 | 核心任务 | 验收标准 |
| :- | :- | :- |
| **Phase 1**：域名与网关 | 域名注册 + ICP 备案 + Let's Encrypt/acme.sh + EdgeOne Makers 开通；`cover_server/` 公网化试点 | 公网 HTTPS 访问封面服务成功；App 内封面键公网前缀开关生效 |
| **Phase 2**：数据底座与监控 | Supabase 建库与认证打通；Sentry 接入 Android 端；TMDB 代理函数上线 | 社区广场读写真实数据；Sentry 收到首条模拟异常；详情页远程评分走 TMDB 代理成功 |
| **Phase 3**：存储与增长 | 七牛/又拍云图床接入；百度统计/51.la 落地页统计；Resend 邮件（按需） | 封面走 CDN；落地页有访问报表；邮件链路实测收发 |

### 46.5 成本合计

- 固定成本：**域名 29 元/年**（含备案所需）；
- 弹性成本：0 元（全部服务免费额度内）；超额风险点：Supabase 500MB、七牛 10GB、Sentry 5000 事件/月——个人规模短期无忧，纳入 Phase 2 上线后的月度巡检项。

---

## 47. P37 v1.0.9 真机反馈批次 (User Device-Feedback Batch)

> 立项时间：2026-09-05，来源：用户真机试用反馈（别人手机安装 ReadTrace_1.0.8-hotfix-0905.apk 后）。共六项：一项功能删除、三项缺陷、两项体验增强。

### 47.1 批次总览

| 编号 | 类型 | 问题 | 优先级 | 状态 |
| :-: | :-: | :- | :-: | :-: |
| F1 | 功能删除 | 删除 Web 微卡功能 | 高 | ✅ 已完成 |
| F2 | 缺陷 | 长卷封面加载过慢；番剧条目在全部分类下无封面（占位符） | 高 | ✅ 已完成 |
| F3 | 缺陷 | 精神档案通行证卡日间文字与背景混淆看不清 | 高 | ✅ 已完成 |
| F4 | 体验 | 全息卡带触发物理反馈不再弹 Toast；同步审计其他触觉+Toast 组合 | 中 | ✅ 已完成 |
| F5 | 增强 | 全息卡带封面支持上下左右调整位置 | 中 | ✅ 已完成 |
| F6 | 缺陷+增强 | 云歌单播放标题/封面仍是内置作品；星空占位图换高级素材 | 高 | ✅ 已完成（云曲真封面待真机 Cookie 实测） |

### 47.2 F1 删除 Web 微卡功能

- **范围定位**：核心实现 `util/InteractiveWebCardExporter.kt`；入口散布于 `BookDetailActivity` / `MemoirFragment` / `HubFragment` / `MediaHubActivity` / `ResonancePosterActivity` 中带「微卡」字样的按钮与分享链路（实施时以 grep「微卡」全量清点）。
- **方案**：删除 Exporter 与全部入口按钮、相关布局节点与字符串；**保留 `readtrace://work/{id}` DeepLink**（Scheme 与 Web 微卡是两套东西，深链仍服务桌面捷径与外部分享）；README/计划中相关表述同步清理。
- **验收**：全库无「微卡」入口残留；分享菜单不再出现 Web 卡片项；编译与冒烟通过。

### 47.3 F2 长卷封面加载慢与番剧封面缺失

- **现象**：全景编年史·阅痕长卷中封面有时加载明显偏慢；「全部」分类下番剧条目封面全是占位符（🌸 EV / 浪客 / 夏目…），番剧分类页亦未显示封面，而书籍等媒介正常。
- **根因初判**：长卷对封面走异步加载，滚动/导出时未等待完成即落位占位符；番剧条目封面键（`covers/netease_…`/LAN 键）可能整类缺失或被 `mediaType` 条件分支跳过——实施时先拉真机 DB 核对番剧条目 `cover_url` 实际值。
- **方案**：① 长卷条目封面加载加「完成后再刷新该行」或导出前预加载 barrier；② 修复番剧封面键缺失/分支跳过问题；③ 占位符作为最终兜底保留。
- **涉及**：`AnnualChronicleStudioActivity.kt`、`CoverImageHelper.kt`、（若键缺失）预设资产。
- **验收**：全部/番剧分类下番剧条目均显示真实封面；长卷滚动与导出无长时间占位。

### 47.4 F3 精神档案通行证卡日间对比度

- **现象**：日间模式下通行证卡（READTRACE PASS / 自由漫游策展人 / SINCE…）浅字压浅底，几乎不可读。
- **根因初判**：`CuratorPassCardView` 文字/描边为固定浅色（为深色卡面设计），日间底色变浅后对比度崩塌——同类问题见已修复的 D 组（新版本弹窗）。
- **方案**：文字、徽标与装饰线全部接入日夜色牌（`readtrace_ink` / `readtrace_glass_stroke` 等），卡面底色日间换浅色变体；对比度目标 ≥ 4.5:1。
- **涉及**：`CuratorPassCardView.kt`、档案页布局。
- **验收**：日/夜双模式截图比对可读性达标。

### 47.5 F4 触觉反馈去弹窗 + 全局组合审计

- **现象**：全息卡带页触发插槽物理反馈时弹出 Toast「🕹️ 触发白金卡带插槽物理反馈」（`GameCartridgePosterActivity.kt:90`）——触觉本身即反馈，Toast 冗余。
- **Toast 审计清单（实施时逐一核对去留）**：卡带页内其余 7 个 Toast 均为动作告知（无库存/已切换/导出中/失败/已保存），保留；其他「触觉+Toast」伴生位点：`CulturalPassportActivity:88`（盖印）、`MovieTicketPosterActivity:96/101/132`（撕票/卡扣）、`MindprintTopologyActivity:175`、`MindprintConstellationActivity:63`、`ResonancePosterActivity:69/129`（星系共鸣）——原则：**动作成功的触觉反馈不配 Toast，仅失败/结果性信息保留**。
- **涉及**：`GameCartridgePosterActivity.kt` 及上述审计位点。
- **验收**：卡带插槽反馈无弹窗；全局无「成功动作仍弹 Toast」残留。

### 47.6 F5 全息卡带封面位置可调

- **方案**：卡面封面区支持拖拽平移（上下左右调整取景位置），双指缩放可选；偏移量按作品持久化（DB 新列或 SharedPreferences 键），导出/分享位图按偏移渲染；提供「双击复位」。
...
- **方案**：① 排查云曲播放链路标题/封面绑定：磁带/黑胶卡面 `trackTitle/artistName`、顶栏副标题、底部播报在 `playCloudTrack` 成功后是否全量刷新，找出仍读 `playlist[currentIndex]` 的残留点；② 用一张高质量星空素材（APK assets 内置，替代程序化 Canvas 星点）替换占位封面，保持深夜氛围并提升质感；素材选取时注意分辨率与体积（≤200KB WebP）。
- **涉及**：`VinylCassettePlayerActivity.kt`、`VinylTurntableView.kt`、新增星空素材资产。
- **验收**：云曲播放全链路（卡面/标题/播报/封面）与实际曲目一致；未取到封面的云曲显示新星空图。

---

## 49. P39 精选热门榜单货架与极速建库轻量化系统 (Curated Shelves & Zero-Friction Onboarding System)

> 立项时间：2026-09-08。来源：用户冷启动摩擦力高、单一搜索源（豆瓣正则网页爬虫）脆弱易封控、录入单部作品繁重、缺乏全网爆款与高光作品聚合。
> 架构对标：Ryot (4.5k★)、bangumi-data (4.7k★)、Letterboxd、Steam 官方开放接口、豆伴/Tofu。

### 49.1 核心痛点与设计原则

1. **从 Search-First（搜索驱动）转为 Browse-First（货架驱动）**：
   - 告别冰冷空白的搜索输入框；进入「发现」页首屏即呈现**高质感横滑策展画廊（Curated Shelf Banner）**，热门与经典作品一触即达。
2. **零摩擦占位原则（Zero-Friction Stashing）**：
   - 导入阶段**只写入客观元数据骨架**（标题、创作者、封面、媒介类型、客观评分、标签、简介）；
   - 个人主观字段（用户评分、感想长短评、阅读起止日期、心智五维）全部允许留白，绝不在初次导入节点阻断用户心流。
3. **批量连击打标（Checklist Batch Tap）**：
   - 浏览榜单支持全屏海报多选，手指顺滑连续勾选，配合线性马达轻颤反馈，2 分钟内可完成 50+ 部经典作品批量建库。
4. **极速微调打分与美学情绪胶囊**：
   - 触觉滑动手势打分（大号星标水平滑动 + 机械棘轮微震 + 滚轮数字），告别繁琐数字输入；
   - 引入预设“美学情绪胶囊”（Vibe Chips），单点代替敲字短评，并自动反向推算五维心智势能。

---

### 49.2 全媒介精选榜单与静态数据源矩阵

在 `app/src/main/assets/curated/` 下规范组织静态策展数据包，零网络依赖、零反爬风险、秒级加载：

| 媒介分类 | 专题名称 | 数据源与格式 | 核心价值与条目规模 |
| :--- | :--- | :--- | :--- |
| **🎬 影视** | **🏆 豆瓣电影 TOP 250** | `douban_movie_top250.json` | 中文影迷人均标记几十部的公认神作库，含中英双语标题、导演、评分、经典短评。 |
| **🎬 影视** | **🌟 IMDb 影史殿堂 TOP 250** | `imdb_top250.json` | 全球硬核影视工业与先锋艺术标杆。 |
| **🌸 番剧** | **📅 2026 当季新番放送周历** | `bangumi_current_season.json` | 引用 GitHub 开源神库 `bangumi-data`，包含周一至周日当季新作，含中文译名、放送平台、制作阵容。 |
| **🌸 番剧** | **🥇 Bangumi 二次元殿堂 TOP 100** | `bangumi_top100.json` | 动漫与轻小说核心爱好者评分最高的神作名录（EVA、钢炼、CLANNAD等）。 |
| **📚 书籍** | **📖 豆瓣读书 TOP 250** | `douban_book_top250.json` | 人文社科、世界名著、小说精选（《百年孤独》《三体》《活着》等）。 |
| **📚 书籍** | **⚡ 雨果奖/星云奖科幻殿堂** | `scifi_awards_classics.json` | 硬核科幻与奇幻文学必读书单（《沙丘》《海伯利安》《银河帝国》等）。 |
| **🕹️ 游戏** | **👑 Steam 历史好评如潮 TOP 100** | `steam_top_acclaimed.json` | Steam250 整理的历史好评率超 95% 的第九艺术杰作（《空洞骑士》《传送门》《黑神话：悟空》等）。 |
| **🕹️ 游戏** | **🏆 TGA 历届年度最佳游戏全集** | `tga_goty_history.json` | 2014~2025 历届 TGA 年度最佳（GOTY）与高光提名作品。 |
| **💿 音乐** | **💽 滚石 500 张史上最伟大专辑精选** | `rolling_stone_top_albums.json` | 流行、摇滚、爵士史诗唱片，呼应 3D 黑胶落针播放器。 |
| **💿 音乐** | **✨ ACG 影视原声与独立音乐专题** | `curated_ost_classics.json` | 久石让、泽野弘之、Yorushika、ZUTOMAYO 等高共鸣曲目专辑。 |

---

### 49.3 发现页（DiscoverActivity）UI 与交互改造方案

1. **DiscoverActivity 顶部增加横滑策展画廊（Curated Shelf Banner）**：
   - 位于搜索框与分类标签栏上方，采用 `RecyclerView` 水平横滑（带轻微景深与边缘半透明渐变）；
   - 呈现 4~5 个特色鲜明的视觉展牌卡片（深灰金曜、极光淡粉、墨绿羊皮纸、霓虹赛博蓝紫）；
   - 轻触任一展牌，即刻展开对应的榜单专题详情页（`CuratedShelfActivity`）。
2. **榜单多选与极速批量勾选流（Checklist Multi-Select）**：
   - 专题内采用 3 列瀑布流海报布局，卡片右上角配备微光勾选圈；
   - 支持手指连续滑动点选，每次点中触发 `HapticFeedback.lightTick`；
   - 底部常驻磨砂玻璃悬浮胶囊：`【 一键导入选中的 N 部作品 】`；
   - 唤起 0.5s 极简底栏，选择一次标记状态（默认「已看/已读/已通关」），单事务批量写入 SQLite 数据库，主线程零卡顿。

---

### 49.4 极速评分与轻量化信息录入（Effortless Rating & Vibe Chips）

1. **触觉滑动手势打分（Haptic Swipe Rating）**：
   - 在作品卡片和详情页提供大号质感星级（5 星 / 10 分双刻度）；
   - 水平滑动手势直接驱动星辉填充与 `RollingNumberTextView` 数字解密滚轮；
   - 每跨过 0.5 星或 1 分刻度，调用物理线性马达产生清脆棘轮咔哒顿挫感（`HapticFeedbackEngine.tick`）；
   - 松手即完成打分持久化，无需弹出输入法键盘。
2. **预设美学情绪胶囊（Vibe Chips）**：
   - 针对不同媒介配置专属高频情绪标签，单选或多选（最多 3 个）：
     - 影视/番剧：`#后劲极大` `#神级反转` `#视听盛宴` `#泪腺崩坏` `#赛博思考` `#意难平`
     - 书籍：`#醍醐灌顶` `#文笔绝美` `#硬核烧脑` `#枕边治愈` `#洞察人性`
     - 游戏：`#神作跪拜` `#电子阳痿解药` `#音乐封神` `#叙事奇迹` `#手残受苦`
   - **心智雷达联动**：选中特定情绪胶囊后，算法自动按权重微调五维心智雷达参数（例如 `#硬核烧脑` 增加理智权重，`#泪腺崩坏` 增加情感权重），免去手动拖拽调节。
3. **快捷时间语义胶囊（Date Semantics Chips）**：
   - 标记为“已看完/已通关”时，默认日期为「今天」；
   - 提供语义快捷胶囊：`[ 今天刚完结 ]`、`[ 昨天 ]`、`[ 上周 ]`、`[ 很久以前（留白） ]`，覆盖 90% 日常记录，彻底摆脱翻年月日日历控件的痛苦。
4. **大众客观分锚定微调（Anchor Rating）**：
   - 从榜单导入时保留 `remote_rating`（如豆瓣 8.8 分）；
   - 用户主观打分初始指针默认停在客观分附近，用户只需左拨（扣分）或右拨（加分），大幅降低从零评估心智阻力。

---

### 49.5 外部生态一键搬家扩展（Importer Ecosystem）

在备份与搬家中心，增加免登录公开接口直连：
1. **Steam 游戏库一键导入**：
   - 输入用户 Steam ID 或 Vanity URL，调用 Steam Web API `GetOwnedGames`；
   - 自动提取游戏库名录、游玩时长，按时长 > 0 自动标记为「在玩/已通关」，批量铸入全息卡带墙。
2. **Bangumi 账号一键同步**：
   - 输入 Bangumi 用户名，调用公开 API v0 `users/{username}/collections`；
   - 一键将用户的在看、看过、搁置、抛弃动漫全量恢复至《阅痕》。
...
| 阶段 | 核心任务 | 交付物与验收标准 |
| :--- | :--- | :--- |
| **Phase 1：静态策展包与货架 Banner** | ① 下载并清洗豆瓣电影250、豆瓣读书250、bangumi-data当季番、Steam好评百大为标准 JSON；<br>② `assets/curated/` 目录组织与解析工具类 `CuratedShelfRepository`；<br>③ `DiscoverActivity` 顶部横滑 Banner 与专题网格展示。 | 离线断网状态下进入发现页，顶部 Banner 秒级滑出，点击进入专题展示 100+ 部高清封面作品，加载耗时 < 100ms。 |
...
| **v1.0.12**| P39 精选热门榜单货架、离线数据集、批量勾选静音化、触觉滑动手势打分、美学情绪胶囊心智联动、极速建库引导与沉淀闭环 | ✅ 已完成 |
...
## 53. P39 精选热门榜单货架与极速建库轻量化系统（v1.0.12）

### 53.1 阶段规划与交付状态

| 阶段 | 核心任务 | 交付状态 |
| :--- | :--- | :--- |
| **Phase 1** | **精选离线热门榜单货架**：内置豆瓣 Top250 书影、Steam Top100、Bangumi 新番、滚石 500 离线数据集；打造发现页横滑专题栏 | ✅ 已完成 (Commit: `23e6ad4`) |
| **Phase 2** | **批量勾选静音化与触觉滑动手势打分**：选择体验彻底剔除马达震动，全选/全清底栏；数据库批量查重与预编译事务；5星10分制 `HapticSwipeRatingBar` | ✅ 已完成 (Commit: `bac690f`) |
| **Phase 3** | **预设美学情绪胶囊与心智雷达动态联动**：书/影/剧/游/音五大媒介高频情绪胶囊矩阵 (`VibeChipEngine`)；详情页点击实时注入/卸载并驱动 `MindprintRadarView` 补间动画；速记弹窗支持情绪胶囊点选并自动生成初始六维心智画像 | ✅ 已完成 (Commit: `8ace443`) |
| **Phase 4** | **一键极速建库引导与无缝沉淀**：极速批量建库流式进度弹窗 (`dialog_fast_library_onboarding.xml`)；完成成果卡片（四宫格统计、均分、流派、心智骨架）；一键直达藏库 (`MainActivity.EXTRA_TAB_INDEX`) 与闭环体验 | ✅ 已完成 (Commit: `1dc96f4`) |

---

## 54. P40 安装包性能瘦身、货架深度筛选与架构治理综合演进计划（v1.0.13）

> 立项时间：2026-09-10。
> 背景与定位：在完成 P39 离线热门货架（336 部作品）与极速建库闭环后，针对当前暴露的「安装包资源冗余过大（28.6MB）」、「336部作品在货架中找寻层级较深」、「`BookDatabaseHelper` 膨胀至 4,200+ 行巨石代码」三大瓶颈展开专项优化；同时补充双作品心智叠合对比与核心算法单测，构建稳健轻盈的客户端基石。（注：经评审，外部第三方账号免登搬家暂不列入本次范围）。

### 54.1 核心攻坚方向与技术方案

#### 1. 📦 安装包极致瘦身（50% Slim Release）
- **预置封面 WebP 现代化压制**：
  - 现状：`assets/covers/` 下 104 张预置封面全为 JPG，占用 14.65 MB（占 APK 总量的一半以上）；
  - 方案：批量进行高质量有损/微损 WebP 压制（质量 80%~85%，等比限制在 720p 移动端合适规格），预计体积直接由 14.65 MB 骤降至 **~3.5 MB**（净减 11 MB+）；
  - 适配：`CoverImageHelper` 增加对 `.webp` 内置资产的优先索引与解码；
- **构建优化与资源裁剪**：
  - 在 `app/build.gradle.kts` 中评估开启 R8 代码混淆 (`isMinifyEnabled = true`) 与资源收缩 (`isShrinkResources = true`)；
  - 补齐 `proguard-rules.pro`（保留反射与 JSON 序列化数据模型）；
- **目标收益**：正式 Release APK 由 **28.6 MB 缩减至 14~16 MB**（体积腰斩近 50%），下载与装机耗时减半。

#### 2. 🔍 离线货架二级流派/年代筛选与微光体验（Discover Shelves Polish）
- **榜单内多维标签筛选**：
  - 现状：336 部作品进入专题后只能单向无限下滑，海量条目挑选效率待提升；
  - 方案：在 `DiscoverActivity` 货架专题列表顶部增加流式筛选胶囊栏：
    - 豆瓣电影 250：增加 `[全部]`、`[科幻]`、`[剧情]`、`[悬疑/犯罪]`、`[80/90年代经典]`；
    - 豆瓣读书 250：增加 `[全部]`、`[世界名著]`、`[社科硬核]`、`[科幻小说]`、`[心理与人生]`；
    - Steam 榜单：增加 `[全部]`、`[肉鸽/动作]`、`[魂类/受苦]`、`[叙事/剧情向]`、`[模拟经营]`；
    - Bangumi / 滚石榜单：对应增加核心子分类过滤；
- **微光骨架屏（Shimmer Loading Placeholder）**：
  - 列表加载与网络图片未完成渲染阶段，加入细腻微光流动骨架屏动效，消除生硬空白与跳变感。

#### 3. 🔮 心智雷达双作品叠合对比 (Mindprint Comparison)
- **多选对比入口**：在藏库列表多选状态下，当用户勾选正好 2 部作品时，底栏点亮「🔮 心智对比」胶囊；
- **双色多边形叠合呈现**：弹出高质感玻璃拟态弹窗，在同一张 `MindprintRadarView` 上以金曜金（作品 A）与极光青（作品 B）双色半透明蒙层叠合对比两部作品在六维心智上的审美张力与差异。

#### 4. 🏛️ 巨石类架构解耦与治理（Architecture Decoupling）
- **现状分析**：`BookDatabaseHelper.kt` 膨胀至 **4,254 行**，聚合了 9 张表 DDL、v1~v13 迁移、统计聚合 SQL、预置种子播种与数据修复；
- **解耦方案**：
  - 对外保留 `BookDatabaseHelper` 现有单例入口与方法调用，保证既有业务调用 100% 零破坏；
  - 内部按领域拆分出轻量组件与 DAO：
    - `BookDao`：作品 CRUD、批量查重、事务落库与状态过滤；
    - `NoteDao`：笔记、摘录、随想与阅读会话管理；
    - `MindprintDao`：六维心智模型、情绪雷达分值存取；
    - `DatabaseMigrator`：抽离 v1 到 v13 的增量升级脚本；
    - `PresetSeedManager`：抽离内置 CSV 种子与富内容恢复逻辑；
- **目标收益**：巨石类体积缩减 60% 以上，职责边界清晰，降低维护风险。

#### 5. 🧪 算法单测兜底与质量加固
- **VibeChipEngineTest**：补全单元测试，严密校验各媒介美学情绪胶囊向六维心智向量累加计算的数学正确性与 0.0~10.0 边界裁剪；
- **CuratedShelfRepositoryTest**：校验 5 大主题离线 JSON（336 部作品）字段完整性，确保 title、category、remoteRating、coverUrl 无空指针或解析异常。

---

### 54.2 实施路线图 (Phases Roadmap)
