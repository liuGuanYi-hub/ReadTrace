...
class HubFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        firstScreenStage.doOnLayout {
            ...
            // 舞台底边贴住屏幕底，滚动位置 0 时第二页内容（Hero 等）完全在屏幕外
    ...
    private fun showImportCsvDialog() {
        ...
        // 现统一由「一键全量合入」从内置 preset_all.json 导入（含完整 mindprint）。
        //
        // 2026-09-19：文案**刻意不再写死作品数**。预置库会随内容增补而变化
        // （加入《进击的巨人》六季后已由 218 部变为 224 部），写死数字每次都要同步，
        // 否则就变成向用户展示错误信息的僵尸文案。
        val options = listOf(
            "🌟  一键全量合入（内置全库精选集）",
            "📂  选择本地 CSV 文件...",
