...
/**
 * 预置播种与数据修复管理器（P40 Phase 4 阶段 2 自 BookDatabaseHelper 抽出）。
 * 只搬代码不改行为：播种调用线程、事务结构、幂等语义一律原样保留。
 */
object PresetSeedManager {

    private const val SEED_PREF = DatabaseSchema.SEED_PREF
    private const val KEY_SEED_VERSION = DatabaseSchema.KEY_SEED_VERSION
    private const val DATABASE_NAME = BookDatabaseHelper.DATABASE_NAME
    private const val DATABASE_VERSION = BookDatabaseHelper.DATABASE_VERSION
    ...
    internal fun backupDatabaseFile(context: Context, version: Int, db: SQLiteDatabase) {
        runCatching {
    ...
    internal fun patchCorruptedPresetCovers(db: SQLiteDatabase) {
        runCatching {
     ...
     */
    internal fun runPresetSeedsOnce(context: Context, db: SQLiteDatabase, onBooksChanged: () -> Unit) {
        ...
        synchronized(seedLock) {
            ...
            if (previousSeedVersion != DATABASE_VERSION) {
                ...
                try {
                    ...
                    populatePresetRichContent(context, db, onBooksChanged)
                    seedCuratedBookCovers(db)
                    migrateCoversToLanKeys(context, db)
            ...
            ensureRichContentSeededIfNeeded(context, db, onBooksChanged)
     ...
     */
    private fun migrateCoversToLanKeys(context: Context, db: SQLiteDatabase) {
        runCatching {
     ...
     */
    internal fun wipeAllUserData(db: SQLiteDatabase, onBooksChanged: () -> Unit): Pair<Int, Int> {
        db.beginTransaction()
        var wipedBooks = 0
        var wipedNotes = 0
        try {
    ...
    private fun assetNameForRemoteUrl(url: String): String? = runCatching {
        ...
        val tag = when {
            ...
            host.endsWith("unsplash.com") -> "unsplash"
        ...
        val last = sanitize(segments.last())
        // steam 封面统一叫 library_600x900.jpg、网易云数字 ID 可能跨哈希重复，均需拼接倒数第二段保证唯一
        val name = if ((tag == "steam" || tag == "netease") && segments.size >= 2) {
            sanitize(segments[segments.size - 2]) + "_" + last
        } else {
            last
        }
        tag + "_" + if (name.contains('.')) name else "$name.jpg"
    }.getOrNull()
     ...
     */
    private fun populatePresetRichContent(context: Context, db: SQLiteDatabase, onBooksChanged: () -> Unit) {
        runCatching {
     ...
     */
    private fun ensureRichContentSeededIfNeeded(context: Context, db: SQLiteDatabase, onBooksChanged: () -> Unit) {
        runCatching {
     ...
     */
    internal fun importRichContentJson(
        db: SQLiteDatabase,
        jsonText: String,
    ...
    internal fun autoFillMissingCovers(db: SQLiteDatabase) {
        runCatching {
