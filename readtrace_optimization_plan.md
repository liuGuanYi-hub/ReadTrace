...
**✅ 已决策（2026-09-21）**：已被 v15 随机改写的评分**不做补偿**，本项关闭。
理由：① 实测本机库中符合"被改写"条件的条目为 **0 部**（非预设来源的 music/movie/game 且评分 ≤ 6.0）；
② v15 用 `ABS(RANDOM())` 且旧值未留底，**无法识别**哪些评分被改过，补偿逻辑无从下手；
③ 唯一可能的恢复来源是 WebDAV/JSON 备份，仅对已备份用户有效，成本远大于收益。
守卫已补回 + 单测已覆盖，问题不会继续扩大；将来若有个案反馈，从备份单独恢复。详见 §10 的关闭说明。
...
>
> **2026-09-18 落实结果**：#8 已解决（改走 `DatabaseMigrator` v18 精确匹配迁移，非一次性 `UPDATE`）；#9 已解决（历史条目保留 + 标「〔已下线〕」）；**#11 已解决**（用户授权后完成 T4.9 门禁改造，未跑 `updateLintBaseline`）。**#10 已决策**（转为触发条件制：不接线也不删除，待"用户实际反馈首次切换卡顿"时再启用）。
>
> **2026-09-21 落实结果**：**#1 已关闭**（用户明确决定：已被 v15 改写的评分不做补偿；实测本机库中符合改写条件的条目为 0 部，且 `ABS(RANDOM())` 未留底导致无法识别受影响数据，补偿不可行）。**至此 §10 全部 14 项决策均已拍板，无遗留待决项。**

| # | 决策项 | 关联任务 | 影响 | 状态 |
|:--:|:--|:--|:--|:--:|
| 1 | 是否授权修改迁移逻辑？是否需要对已被 v15 改写的评分做补偿？ | T0.1 | 历史数据 | ✅ **已决策（2026-09-21，用户明确）**：迁移逻辑已修（守卫补回 + 单测断言）；**已改写的评分不做补偿，此项关闭**。理由见本节末尾注 |
...
| 10 | `MainActivity.preloadRemainingTabs()` 是**接线**还是**删除**？（零调用，但它是 T0.3 播种后台化后配套的冷启动预热，接线可能仍有收益） | T4.5-C | 冷启动性能 | ✅ **已决策（2026-09-18）：既不接线也不删除，转为触发条件制**。收益未量化（"消除首次切换卡顿"属推断，而本项目 T2.5 的模拟器实测为滚动卡顿率 0%，说明性能并非瓶颈），风险却明确（启动时多创建 4 个 Fragment 的视图与查询）。**保留定义**，待「用户实际反馈首次切换 Tab 卡顿」时再接线；它带 `postDelayed` 错峰与 `currentTabIndex` 守卫，接线条件已就绪 |
...
### 2026-09-18 T4 收官阶段新发现的决策项（四条均已拍板）

| # | 事项 | 结论与理由 |
|:--:|:---|:---|
| 12 | 根目录 `912_filled.json` 与 `assets/preset_all.json` 内容不一致——前者仍是「**展厅**第5层」，后者已按 T4.5-A#3 修正为「**馆藏**第5层」，其余字节完全相同 | ✅ **已删除该文件**。它是 `tools/backfill_mindprint.py` 的输出产物（输入为同目录 `912.json`、输出 `912_filled.json`，脚本注释明确「绝不覆盖原始文件」），随时可重建；而它躺在项目根目录，存在被误认为权威数据源、进而**回退 T4.5-A#3 修复**的实际风险。`912.json`（backfill 的输入、原始导出的唯一副本）保留原位。<br>⚠️ **后续注意**：若确需重建 `preset_all.json`，**不可直接 `cp 912_filled.json`**——其陈列位置文案落后权威源 137 条 |
| 13 | baseline 中尚有 **45 条**「已登记但工程中不存在」的过期豁免（gradle 构建时提示） | ✅ **决定不处理**。查清成因后认为清理风险大于收益：这 45 条大概率是**行号漂移**所致（T4.3~T4.7 的一批改动移动了代码行），而非问题真的消失——直接删除豁免会让对应问题重新报出并中断构建。安全的做法只有「重新生成 baseline」，但那会把 T3.1 之后新积累的存量告警一并豁免，与 T4.9 的初衷相悖。**当前门禁行为正确，维持现状** |
| 14 | `assets/preset_books\|anime\|movies\|games.csv` 四个旧数据源（共 205 部、84KB）在全部 Kotlin 中引用数为 0 | ✅ **决定保留**，改任「CSV 导入格式范例」。理由：`BookCsvParser` 与「用户自选本地 CSV 导入」的功能**仍然存活**，这四个文件是现成的字段格式参考，84KB 可忽略。<br>⚠️ 本条**不是遗漏**——lint 不扫 `assets/`，这类只能人工发现，特此登记以免下次被当成新问题重复排查 |

### #1 关闭说明（2026-09-21，用户明确决定）

**结论：不追究已被 v15 改写的存量评分，不做补偿逻辑。该决策项就此关闭。**

支撑这个决定的实测依据（模拟器实库查询：`run-as` 拉库 + 本地 sqlite3 直查）：

| 检查项 | 结果 |
|:---|:---|
| 非预设来源（`source_type` 非空）的 music/movie/game 条目 | **0 部** |
| 其中评分 ≤ 6.0 的（v15 的实际作用对象） | **0 部** |
| 对照：预设条目评分区间 | game 7.5~9.9 / movie 7.8~9.3 / music 8.1~8.9 |

即本机数据中**没有任何条目符合"被改写"的条件**。

补偿本身也缺乏可行性：v15 用的是 `ABS(RANDOM())`，**旧值没有留底**，
无法区分"7.3 这个分是原始评分还是被改写的"；唯一可能的来源是 WebDAV/JSON 备份，
而那只对"恰好备份过"的用户有效——为一批**无法识别**的数据写补偿逻辑，复杂度远大于收益。

**已做的兜底**：迁移守卫已补回（v14/v15 遗漏的那行 `source_type` 限定）+ 单测断言，
问题不会继续扩大。若将来真收到"我的评分变了"的用户反馈，再按个案从备份恢复。

> ⚠️ **以后不要再把此项当作"待决策"重新提起**——本次已明确关闭。
